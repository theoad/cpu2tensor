...
line coverage. A separate job with narrowly scoped write access updates a small
SVG on the `ci-badges` branch after successful `main` runs. Pull-request code
retains read-only access, and badge updates do not trigger this workflow.
